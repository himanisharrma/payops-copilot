...
// Compose the wedge breakdown from window deltas.
//
// Closing math: `opening + collections - payouts = closing`. This is
// the canonical merchant_payable identity — captures CR merchant_payable
// gross; payouts DR merchant_payable net. The closing IS the
// merchant_payable balance at asOf.
//
// MDR / GST / refund / chargeback / holds / releases are NOT in the
// closing math; in v1 they touch provider_receivable / fee_expense /
// gst_liability rather than merchant_payable (PayOps is a SaaS, not a
// money-mover). They display as *explanations of the gap* — i.e., "of
// the ₹23.60 unrealized, ₹20 was MDR + ₹3.60 was GST." This is the
// wedge UX: controllers see WHERE the gross was consumed.
//
// Invariant we expose to the UI: `mdr + gst + refund + chargeback +
// holds - releases` should approximate `collections - payouts -
// (closing - opening)` for a fully-settled period. Drift surfaces a
// real recon issue.
export function composeFormula(input: {
  ...
  const closing =
    input.openingPayable
    + input.windowDeltas.collections
    - input.windowDeltas.payouts;
  return {
    openingPayable: round(input.openingPayable),
    collections: round(input.windowDeltas.collections),
    mdr: round(input.windowDeltas.mdr),
    gst: round(input.windowDeltas.gst),
    refund: round(input.windowDeltas.refund),
    chargeback: round(input.windowDeltas.chargeback),
    holds: round(input.windowDeltas.holds),
    releases: round(input.windowDeltas.releases),
    payouts: round(input.windowDeltas.payouts),
    closingPayable: round(closing),
  };
}
